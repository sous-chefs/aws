...
action :enable do
  unless monitoring_enabled?
    converge_by('enable monitoring for this instance') do
      ec2.monitor_instances(instance_ids: [ec2?['instance_id']])
    end
  end
end
...
action :disable do
  if monitoring_enabled?
    converge_by('disable monitoring for this instance') do
      ec2.unmonitor_instances(instance_ids: [ec2?['instance_id']])
    end
  end
end
...
action_class do
  ...
  def monitoring_enabled?
    monitoring_state = ec2.describe_instances(instance_ids: [ec2?['instance_id']])['reservations'][0]['instances'][0]['monitoring']['state']
