property :network_interface_id,  String, name_property: true
...
action :attach do
  ...
  if net_interface.nil?
    raise "Elastic Network Interface #{network_interface_id} does not exist"
    ...
    if net_interface[:attachment][:instance_id] == instance_id
      ...
      raise "Elastic Network Interface #{network_interface_id} is attached to another instance"
    end
  else
    ...
    begin
      Timeout.timeout(timeout) do
        loop do
          net_interface = get_network_interface(network_interface_id)
          if net_interface.nil?
            raise 'Elastic Network Interface has been deleted while waiting for attachment'
          elsif net_interface[:attachment].nil?
            next
